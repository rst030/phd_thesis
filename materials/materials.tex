\chapter{Materials, Devices and Experimental Details}
...
\subsection{PDiTBuS Electrochemical Cell for Ex Situ EPR Spectroscopy}
\label{fab:ex_situ_charging}
PDiTBuS was grown as a thin film on an on-substrate flat 3$\times$4~mm$^2$ Au working electrode (WE) of a three-electrode cell as depicted in Figure~\ref{fig:Figure_1}d, by means of electrochemical polymerization. The on-substrate WE was a 180~nm layer of Au deposited on a 3.5~mm wide quartz substrate using metal \ik{evaporation} through a shadow mask, with a 7~nm Cr adhesion layer. For the electrochemical polymerization, the electrochemical cell was filled with DiTBuS monomers dissolved in the electrolyte (0.1~\si{\Molar} Acetonitrile solution of Tetraethylammonium Tetrafluoroborate)\q{,} and the voltage between the WE and the Pt counter electrode (CE) was repeatedly cycled between -200~mV and 800~mV with respect to the calibrated, Ag/10~\si{\milli\Molar}  AgNO$_3$ (Acetonitrile) reference electrode (RE) at a rate of 50~mV~s$^{-1}$ (for the cyclic voltammograms recorded during electrodeposition \ik{see ESI, Figure~\ref{fig:Figure_S1}}). After 50 electropolymerization cycles the pDiTBuS film had a capacity of 1.6~$\muup$Ah with an estimated mass of 20$-$30~$\muup$g and thickness of 800$-$1000~nm \ik{(see ESI, Section~\ref{si:ESOC} for calculations)}\\

The pDiTBuS film was brought to a desired charge state with the electrochemical setup shown in Figure~\ref{fig:Figure_1}d. The container was filled with pure electrolyte (without DiTBuS monomers) and a static current of \q{$\pm10$}~$\muup$A was applied between WE and CE for lifting or lowering the electric potential of the film. The potential of the film during charging and discharging was recorded with respect to the RE as the Galvanostatic Charge-Discharge (GCD) curve (Figure~\ref{fig:Figure_1} and Figure~\ref{fig:Figure_S3} in the ESI). When the GCD reached a desired potential, the WE was disconnected from the current source and the substrate with the pDiTBuS film was removed from the cell. The substrate was dried in pre-vacuum and flame sealed in a 5~mm OD quartz tube under 500~mbar of He. The tube was then  frozen in liquid N$_2$ (77~K) and transported to the pre-cooled EPR spectrometer. In order to alter the \ik{SoC} of the film between the EPR measurements, the tube was gradually warmed up to room temperature and opened, then the substrate \ik{with the charged film} was removed from the tube and placed to the electrochemical setup where it was fully \ik{charged from the preserved SoC} to 700~mV, and discharged to the \ik{next} desired SoC galvanostatically (ESI, Figure~\ref{fig:Figure_S27}). After charging, the substrate was dried and encapsulated as described. SoC corresponding to -200~mV, 430~mV, 500~mV and 590~mV vs. the RE were considered. The potentials correspond to 0\%, 65\%, 95\% and 98\% of the full \ik{1.4}~$\muup$Ah \ik{discharging} capacity of the film, determined with Coulomb counting~\cite{Movassagh2021}. (\ik{The charging capacity of the film has decreased by 12\% upon repeated charging and temperature cycling,} cf. ESI, Figure~\ref{fig:Figure_S2} and Section~\ref{sample_fab_2}).\\
